...
캐싱의 목적은 Cache에 데이터의 복사본을 저장해놓고 이를 읽거나 씀으로써 비교적 읽기/쓰기 비용이 큰 장치로부터 접근횟수를 줄여 비용을 아끼는 것이 주 목적중 하나이다. 

이를 통해 속도 최적화나 부하 분산이라는 성능 향상 목표를 달성할 수 있다.
...
### 성능 향상
캐싱하는 이유 뭘까 -> 성능 향상 
	응답속도 개선을 통한 -> 빠른 응답으로 달성 가능 -> 빠른 응답을 하기 위해선 -> 데이터를 빠르게 가져올 수 잇어야함. -> 데이터의 연산이 이루어지는 곳과 데이터가 가까운 곳에 위치해야함.
...
어떤 데이터들이 캐싱되면 성능향상에 도움이 될지를 고려해야한다. 
아무런 기준 없이 랜덤한 데이터들이 캐싱되면 성능이 향상될까?

어떤 데이터가 캐싱되어야하는지가 왜 중요한지를 이해하기 위해서 캐시의 효과를 측정하는 방법 대해서 먼저 알아볼 필요가 있음. 캐시가 효과적으로 동작하고 있는지를 다음과 같은 성능 지표로 확인해볼 수 있음. 

캐시 적중(Cache Hit) : 캐시에 요청한 데이터가 존재하는 비율
- 캐시 적중률이 높다면 캐시가 잘 이용되고 있다는것, 효율적으로 운용되고 있다는 것.

캐시 미스 (Cache Miss) : 캐시에 요청한 데이터가 존재하지 않는 비율
- 캐시 미스가 높다면 캐시에 원하는 데이터가 없었다는 것.
...
물리 저장 장치의 경우 공간이 한정되어있다. -> 모든 데이터를 전부 캐싱할 수는 없음 (현실적으로 비용문제가 큼.)

한계점으로 고민점이 생김 -> 그러면 어떤 데이터들이 캐시 공간에 남아있어야 하고, 어떤 데이터들이 내려가야하는지.
